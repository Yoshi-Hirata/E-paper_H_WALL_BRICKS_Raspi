...
repo = Path(sys.argv[1]); url = sys.argv[2]
sys.path.insert(0, str(repo))
from conductor.look import Design, LookMap, check, default_shift  # noqa: E402
...
def grid_csv(look_map, colour_of):
    ...
    for side in look_map.sides:
        ...
        for row in rows:
            ...
            lines.append(f"{side},{row},{default_shift(row)}," + ",".join(cells))
    return "\n".join(lines) + "\n"
