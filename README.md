...
- 検証(`conductor/timeline.py`): 同じ機体の書き換えは、送信から次の送信まで
  「書き換え時間 + ディレクターの余裕 1 秒」と「基板を実際に書き込む時間(本体側
  ui/showplay.py と同じ定数: 基板数 × 0.25 秒 + 2 秒。スイープするキューは
  delay table も書くので 2 倍)」の大きい方が必要(7 秒refresh なら基板 24 枚まで
  8 秒、32 枚で 10 秒、36 枚で 11 秒)。プリセット直後の最初のキューはさらに、
  本体が `/show/run` でちょうど合流し直している場合を見込んだ下限(書き込み +
  セットアップ 1 秒 + 全基板確認 + 1 秒)を取る。この詰めた間隔は実機でまだ
  確認できていない(docs/STATUS.md 参照)。同じ機体に載るアイテム(Look20 の
  上下)は同時刻なら 1 回の書き換え、ずらすなら上の間隔が要る。ほかに、未取り込みのデザイン、
