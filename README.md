...
**ショーの進行**(機体タブの上段、P2): タイムラインを機体ごとの
**ショーファイル**にまとめて配り、あとは各機体が自分の時計で実行する。
本番中に PC や Wi-Fi が落ちてもショーは最後まで進む。

| 操作 | 動作 |
|---|---|
| ① 配布 | タイムラインに問題が無ければ、機体ごとのショーファイル(全キューの配列 + 各キュー後の全体像)を送る。機体はディスク(`~/.epaper/`)に保存 |
| ② プリセットを表示 | 0:00 の姿を先に出しておく(START で書き換えが起きない) |
