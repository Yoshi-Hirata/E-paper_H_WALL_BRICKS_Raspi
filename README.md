...
- トラックをクリックでキュー追加、キューをクリックで **Start(変化の開始)/ End(完成)**・
  デザイン・一部更新・**遷移(Transition)** を編集。End を入れると「その時刻に完成」、
  Start を入れると遷移なしのキューは「その時刻に開始」、遷移つきのキューは End を保ったまま
  Span を選び直して開始を合わせる(0〜30 s に収める)。**Fixed** は書き換え時間を変えたときに
  動かさない側(end / start)
