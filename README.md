...
**スタンドアロンデモ(STANDBY のすぐ下に並ぶ行)は再起動・停電のあとも
自動で再開する。**止めるのは **KEY2** か PC の **STOP** ―― 再生中に機体が
落ちても、電源が戻れば同じデモが同じ位置から続き(ループ設定も保たれる)、
待機の白で塗り潰されることはない。デモ行の KEY1 が断られるのは PC が
その機体を握っているとき(ショーが再生中・保留中、再起動で絵が戻ったまま
待機中、絵を書き込み中)だけで、理由がメニューに 5 秒出る。
...
- **機体への書き込み**: ツールバーの **Write to units…**(青いボタン。Units タブの
  「THE SHOW」カードの **④ Save on units (plays without this PC)**、および
  「PLAY WITHOUT THIS PC」カードのボタンも同じダイアログを開く)。いま見ている
  タイムラインを機体へ送る **2 つの道**を並べて示し、それぞれ一文で結果を説明する:
  - **Upload for the show** … いま全部の絵を基板へ焼き込み、タイムラインを機体へ渡す。
    以後この PC が START / HOLD / NEXT / STOP を出す(= ① Upload と同じ動作)
  - **Save on the units** … 同じタイムラインを各機体自身のメニューに名前つきで保存する。
    機体は KEY1 でそれを再生する(PC 不要)
  かかる時間(1 枚 0.31 秒 × いちばん重い機体の枚数)、送り先の機体一覧(online /
  offline / デモ再生中)、押せない理由(キューが無い・タイムラインに問題がある・
  ショーが進行中・デモ再生中・名前が未入力・名前が機体の LCD で出せない)をその場に書く。
  - **どのルックを書くか**: ダイアログ上部の **WHICH LOOKs** のラジオで「All LOOKs」
    (既定)か LOOK 1 つ(その LOOK の機体だけ。上下キーでも選べる)を選ぶ。1 つを選ぶと
    以下の送り先・所要時間・結果がその機体だけになり、書かない機体は
    「Left as they are: radxa-04 · older upload」として残る(=これが下の警告の中身)。
    **START と ② Show preset は、タイムラインの全機体が同じ Upload を持っていないと
    断る**(`radxa-04 is not on this upload - Upload for All LOOKs before the show`。
    リハーサルなどで承知の上なら確認ダイアログで先へ進める)。1 ルックだけの Upload は
    確認用で、本番前には必ず All LOOKs で Upload し直すこと(チップが `uploaded 1/6` の
    ままならまだ全機体に行き渡っていない)。ショーの**長さが変わった**ときは 1 ルックだけの
    Upload は拒否される(機体ごとに長さが違うショーになるため。All LOOKs でやり直す)
  1 台も応答していないときは Upload の側にもその旨が出る。実行後は機体ごとの結果
  (成否を見出し自身が言う。1 台も書けなければ赤)と、デモなら機体側の手順
  (**KEY2** でメニュー → **名前**(STANDBY のすぐ下)→ **KEY1** で再生、**KEY2** で停止)を
  出す。Esc・背景クリック(書き込み中は無効)で閉じ、Tab はダイアログの中だけを回り、
  名前欄で Enter を押すと「Save on the units」になる
- **いまタイムラインが機体に入っているか**は、Timeline の「THE LOOKS AT」バーと
  Units タブの「THE SHOW」見出しに出る 2 つのチップで分かる:
  `uploaded 10/10 · up to date` と `demo PARIS SS26 10/10 · changed since`
  (狭い画面では判定の語を省き、色とツールチップで示す)。判定は 2 つの照合の
  **両方**が揃ったときだけ言う:
  - **機体が持っているものは、この PC が送ったものか** ― 機体が報告する
    `units[].show.id` ・ `units[].demos[].show_id` と `/api/fleet` の `shows[unit].id`
  - **送ったものは、いま画面にあるタイムラインか** ― ワークスペースの版
    (`/api/fleet` の `timeline.revision` = show.json と全 CSV の指紋)と、
    書き込んだ時点の版(`timeline.uploaded` ・ `timeline.demos[名前]`)。
    id だけでは分からない(機体の id はこの PC が送った id そのものなので、
    そのあといくら編集しても一致したまま)。サーバが覚えているので、ページを
    再読み込みしても、別のブラウザで開いても同じことを言う。
    conductor を再起動すると「分からない」に戻り、判定の語は出さない
...
- **演出家に渡すシミュレーター**: **Simulator for designers…** で、いまワークスペースに
  入っている音楽を埋め込んだ 1 ファイル版シミュレーター
  (`az27ss-simulator-YYYYMMDD-with-music.html`、17.5 MB の MP3 で 23 MB 前後)を
  ダウンロードする。演出家はこれをダブルクリックするだけで、音源を選ぶ操作なしに音が鳴る。
  **音源を差し替えたら、ここで作り直して渡す**(2 回目以降のクリックはキャッシュから即座)。
  音源を変えた直後の 1 回だけ、組み立ての間この画面が 1.6 秒ほど固まる。
  音楽が未アップロードのときは音源なしの版が落ちてきて、その旨トーストで出る
...
- **色データは Upload の時点で基板に書き込み済み**(2026-09-24、依頼者の方針:
  ...
  ショーが動いている間の Upload は確認ダイアログ付き(`force`)で、絵を
  失った機体(ショー中の再起動、焼き込み中の STOP)を復帰させるためだけに
  使う - すでに同じショーを持っている機体は 1 枚も書かない(0.02 秒で終わる。
  実測 2026-09-25)。挿入で途中のキューの slot 番号がずれると全体を焼き直す
  (36 基板 × 10 キューで約 112 秒、× 18 キューで約 195 秒)ため、末尾への追加や
  その場の編集の方が速い。Units タブの各機体タイルに **Pictures** 行
  (writing n / N・written・`n of N pictures not written`・
  `not written (cancelled: 理由)`)が出て、「THE SHOW」カードにも
  「pictures written on n / m units」が出る(burn を報告している機体のみを
  分母にする)。START は
...
| ④ Save on units (plays without this PC) | 「Write to units…」ダイアログを 2 つめの選択肢(機体のメニューへ保存 = スタンドアロンデモ)で開く。①〜③ が「この PC が動かすショー」なのに対し、④ は「PC なしで機体だけが流すもの」 |
...
**スタンドアローンデモ**(= 「Write to units…」ダイアログの 2 つめの選択肢
**Save on the units**。Units タブの「PLAY WITHOUT THIS PC (DEMO STORED ON THE
UNITS)」カードのボタン、または THE SHOW の ④ から開く):
いまのタイムラインを機体ごとのショーファイルとして各機体自身のメニューに書き込む。
以後はその機体単体で(この PC なしに)自分の分だけ再生できる。名前はダイアログで
決める(**A〜Z・0〜9・記号、最大 14 文字** — 機体の LCD フォントは日本語を描けないため、
日本語や 15 文字以上は拒否される)。書き込みはタイムラインに問題が
残っている間(① Upload と同じ判定)、名前が空の間、**ショーが進行中の間**
(`/api/fleet/write_demo` も `stop the show first` で断る。Upload と違い `force` は無い)、
デモ再生中の機体がある間は押せず、その理由がボタンの真上に出る。
「Loop」を付けると終了 5 秒後に自動で最初から再生し直す。書き込み結果は
ダイアログに機体ごと 1 行(✓ written / FAILED)と、機体側の操作手順が出る。
「PLAY WITHOUT THIS PC」カードの「Demos on the units」表は各機体の `GET
...
残っている間は `older` ではなく「—」)。機体のタイルには「**On unit**」行
(`PARIS SS26 (4 cues · 1:30 · loop)` のように、その機体が PC 無しで流せるデモそのもの。
1 つも無ければ `no demo stored`、旧いエージェントなど分からないときは数だけ / 「—」)。
表の取得に失敗した機体は
...
- 機体側の再生: LCD メニューの STANDBY のすぐ下にデモの行が並ぶ(`ui/demos.py` ・
  ...
  **再生中に機体が再起動・停電しても、戻ってきたらそのまま再生が続く**
  (`show-run.json` にデモの素性を残している)。止めるのは KEY2 か PC の STOP
