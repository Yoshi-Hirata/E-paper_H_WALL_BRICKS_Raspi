...
- **色データは Upload の時点で基板に書き込み済み**(2026-09-24、依頼者の方針:
  ...
  スロット **1〜19** へ焼き込む(スロット 0 は白のスタンバイ専用)。ショー実行中の
  送信はスロット番号だけを持つトリガ 1 発になり、そこでは何も書き込まない。
  Units タブの各機体タイルに **Pictures** 行(writing n / N・written・FAILED)が出て、
  「THE SHOW」カードにも「pictures written on n / m units」が出る。START は
  書き込み中またはどれかの基板が失敗した機体がある間は機体名つきで拒否される
- 検証(`conductor/timeline.py`): 書き込みはもう実行中に起きないので、同じ機体の
  ...
  「本体の合流待ち」ぶんの上乗せはなくなった)。一方で基板が持てる絵は 19 枚
  (スロット 1〜19。0 は白)までなので、1 機体のタイムラインが 20 枚目の絵を要求すると
  問題として出る(プリセットも 1 枚に数える)。同じ機体に載るアイテム(Look20 の
  上下)は同時刻なら 1 回の書き換え=1 枚、ずらすなら上の間隔が要る。ほかに、
