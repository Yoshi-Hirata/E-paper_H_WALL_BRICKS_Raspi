...
- トラックをクリックでキュー追加、キューをクリックで **Start(変化の開始)/ End(完成)**・
  デザイン・一部更新・**変化の順序(Sequence)** を編集。End を入れると「その時刻に完成」、
  Start を入れると順序なしのキューは「その時刻に開始」、順序つきのキューは End を保ったまま
  Step を選び直して開始を合わせる(Step は 0.1 s 刻みなので数秒ずれることがあり、編集欄に
  実際の長さが出る)。**Fixed** は書き換え時間を変えたときに動かさない側(end / start)。順序は 6 種:
  ソケット順(現状の P01→P60)/ 図心から放射状 / 上→下 / 下→上 / 左→右 / 右→左
  (左右は客席から見た向き、面ごと)。**Step** は行(列・リング)ごとの間隔(秒、既定 0.1)。
  順序つきのキューは「書き換え 7 秒 + 最後の行の遅延」だけ長くかかり、帯もその分伸びる。
  実機で動かすには基板 FW **FW_260923(V1.4)以降**が要る(コマンド 0x1F / 0x25、
  `docs/FW_REQUEST_SEGMENT_DELAY.md` の冒頭に回答の要点)。古い FW では順序は無視され、
  従来どおり P01→P60 で変わる(機体の `/status` の `no_sweep` にその基板が出る)。再生位置を動かすと、その時刻の各アイテムの
