...
- **機体への書き込み**: ツールバーの **Write to units…**(青いボタン。Units タブの
  ...
  ショーが進行中・デモ再生中・名前が未入力)をその場に書く。実行後は機体ごとの結果と、
  デモなら機体側の手順(KEY2 でメニュー → DEMO → 名前 → KEY1 で再生、KEY2 で停止)を
  出す。Esc・背景クリックで閉じ、名前欄で Enter を押すと「Save on the units」になる
- **いまタイムラインが機体に入っているか**は、Timeline の「THE LOOKS AT」バーと
  ...
  「up to date」は確かなときだけ出す ― 機体が報告するショーの id とこの PC が
  書いたショーの id の一致(`/api/fleet` の `shows` と `units[].show.id` ・
  `units[].demos[].show_id`)、それにこのページ自身が「書き込んだあとタイムラインを
  編集したか」を覚えている範囲(ページを再読み込みすると id の比較だけに戻る)
