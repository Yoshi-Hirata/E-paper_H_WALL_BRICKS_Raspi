...
残っている間(① Upload と同じ判定)と名前が空の間は押せない。「Loop」を付けると
終了 5 秒後に自動で最初から再生し直す。書き込み結果は機体ごとに 1 行(OK / FAILED)。
下の「Demos on the units」表は各機体の `GET /demo/list` を集めたもので、名前・
キュー数・長さ・ループ・保持している機体・削除ボタンを 1 デモ 1 行で示す
(Refresh で再取得。書き込み・削除のあとは自動で更新)。**Timeline** 列は、その
デモが書き込まれた時点のタイムラインといまのタイムラインが一致するか
(`current` / `older`)を機体ごとの版で照合したもの — マップやデザイン、
タイムライン自体を変えたあとに再取込みが要るかがここでわかる。機体のタイルにも
「Demos」行(保持しているデモの数。旧いエージェントは値を返さないので「—」)。
...
- **PC のショーとは独立**: デモの再生中でも PC の Upload / START はその機体には
  届かない(機体が `/show/load` `/show/run` を拒否する)。Units タブでその機体の
  「Show」行に `demo` と出ていたら、まず **STOP** を押す(PC 側の進行を止めるだけ
  でなく、機体のデモも一緒に終わらせる)
