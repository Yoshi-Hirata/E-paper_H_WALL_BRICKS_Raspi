...
| ボタン入力 | **8 種 + KEY1 長押しの全てを検出**。長押し後に短押しが二重発火しないことも確認 |
| パネル基板 | `/dev/ttyACM0`(0483:5740)として認識。**通信は `dialout` 追加後** |
| LCD | **SPI オーバーレイ有効化後**(要 sudo + 再起動) |

計算で導いたライン番号(PB7=39、PL5=5、PJ25=313 など)が**実機で全て有効**であることが確認できた。
