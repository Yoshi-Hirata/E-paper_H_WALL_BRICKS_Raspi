...
OTA 状態照会(0x29)を送り、応答した基板を 1 行ずつ列挙する(485 ケーブルを
繋いだままなら壁全体の一覧になる)。表示は次のいずれか:

| 表示 | 意味 |
|---|---|
| `FW_260917` など(緑) | 同梱イメージと size/CRC が一致。緑は最新の同梱版 |
| `V1.1 16-color, build unknown` | 16 色 FW だが size=0、どの版かは不明 |
| `V1.1 size=… crc=…` | 16 色 FW で、同梱していないイメージ |
| `V1.0 6-color (no OTA)` | 旧 6 色 FW(0x29 を知らない) |

プロトコルにバージョン照会コマンドは無く、0x29 が返す「最後に 0x26 で渡された
size/CRC」を指紋にしている。基板が再起動後もこの値を保持するかは**未確認**で、
保持しない FW では 16 色版はすべて `build unknown` になる(その場合は
メーカーにバージョン照会コマンドを依頼する)。UP/DOWN でスクロール、KEY1 で
