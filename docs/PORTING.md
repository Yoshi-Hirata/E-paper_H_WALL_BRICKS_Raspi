...
### 解決済み: USB ホスト構成(2026-08-05、[Radxa 公式ドキュメント](https://docs.radxa.com/en/cubie/a7z/hardware-use/type-c)で確認)

A7Z の Type-C は 2 口とも OTG 対応で、**電源とホストを別ポートに分けられる**。
Pi Zero 2 W(電源用 micro-USB + OTG 用 micro-USB)と同じ使い方ができる。

| ポート | 役割 | 本システムでの使い方 |
|---|---|---|
| USB 2.0 Type-C | 電源入力(既定はデバイスモード)。FEL 書き込みにも使用 | **電源**(PD アダプタ推奨) |
| USB 3.1 Type-C | DP Alt Mode / OTG。**標準的な USB ホストとして動作可能** | **パネル基板の接続先**(rsetup でホストモードを有効化 → 再起動) |

補足:
- ホストモードの有効化は `rsetup`(USB 2.0 側を使う場合は
  `usb_port_type = <0x1>` のオーバーレイ)。**設定 + 再起動が必要**で、
  差すだけでは認識しない点に注意
- パネル基板側も USB-C なので **C-to-C ケーブル**、または
