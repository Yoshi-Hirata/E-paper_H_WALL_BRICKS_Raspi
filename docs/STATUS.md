...
- **正規化は NFC だけ**(合成のみ。**NFKC は使わない**)。`AZ271SD1305_１_HW.csv`
  ...
  **NFD**(「が」= か + ゛)を同じ名前として 1 ファイルに落とすため
  - Conductor: `safe_name()`(NFC → 文字クラス)を 1 か所に用意し、
    `Workspace.save()` と音源の保存が使う。許す文字は**任意の文字体系の
    文字と数字** + `. - _` と空白(`\w` は Unicode 対応)
  - シミュレーター: `conventionalName()` / `renameOntoItem()` の `nfc()`。
    両言語とも goldens の `names` ケースで固定
- **取り込み**: 合成(NFC)だけが許される変更。変わったものは結果の `renamed` に
  `{元の名前: 直した名前}` で載り(操作画面のトーストにも出る)、その名前を指す
  **キューの `design` と `transitions` のキーも一緒に書き換える**ので参照が切れない。
  合成しても使えない名前(パス、制御文字、全角スラッシュ `／`)は今までどおり
  「unusable file name」で断り、勝手に別のファイル名に化けさせない
