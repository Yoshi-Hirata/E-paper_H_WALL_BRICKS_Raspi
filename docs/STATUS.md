...
- **シミュレーター**: CSV を受け取った時点でファイル名を **NFKC** 正規化する
  (`conventionalName()` / `renameOntoItem()`)。全角の数字・英字は半角になり、
  かな・漢字はそのまま
- **Conductor**: `safe_name()`(NFKC → 文字クラス)を 1 か所に用意し、
  `Workspace.save()` と音源の保存が使う。許す文字は**任意の文字体系の
  文字と数字** + `. - _` と空白(`\w` は Unicode 対応なので「柄A」「夏_2」が
  そのまま通る)
- **取り込み**: 名前の**綴り直しだけ**が許される変更。綴り直したものは
  結果の `renamed` に `{元の名前: 直した名前}` で載り(操作画面のトーストにも
  出る)、その名前を指す**キューの `design` と `transitions` のキーも
  一緒に書き換える**ので、参照が切れない。綴り直しても使えない名前
  (パス、制御文字、NFKC で `/` になる全角スラッシュ)は今までどおり
