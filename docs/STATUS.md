...
- **デモの素性をショーと一緒に保存する**。`show-run.json` に
  `demo` / `demo_name` に加えて **`demo_slug`**(どのメニュー行か)を書く。
...
- **LCD が自分のデモを取り戻す**。`ui/main.py` は `restore()` のあとに App を
  ...
  デモは DEMO 画面・`loop` フラグ(メニュー行から)・KEY1 hold の再スタート
  ごと引き継ぐ。読み込んだだけで戻ったデモはメニューのまま(ガーメントには
  何も出ていない)
- **KEY1 のガードを本当に必要な場合だけに**。断るのは **PC のショーが
  running / holding** のときだけ ― そのときだけ PC が本当に握っている。
  loaded / stopped / ended の PC ショーはデモで上書きしてよい(PC の次の
  `/show/load`、あるいは監視の id 不一致リロードが戻す)。注記は **5 秒**に
  伸ばした(オペレーターは 1.3 インチの画面ではなく壁を見ている)
...
- テスト:`tests/test_ui_demos.py` に 6 件(走行中の再起動 → デモのまま再開・
