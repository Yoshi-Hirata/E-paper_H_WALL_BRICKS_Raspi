...
現象:オペレーターがスタンドアロンデモ(デモ行の KEY1)を再生中に USB を
抜き差し → `epaper-ui` が再起動。`ShowPlayer.restore()` はそのデモを
**ただの PC ショー**として戻していた(`/status`: `state: loaded`,
`demo: false`, `burn: burned 256/256`)。以後 `_enter_demo()` のガード
(「PC のショーが STOPPED でなければ断る」)が**どのデモ行の KEY1 も**
断り、注記は 3 秒で消えるので LCD は「何も起きない」ように見えた。
PC から `/show/stop` を打つまで戻らなかった。

直したもの:

- **デモの素性をショーと一緒に保存する**。`show-run.json` に
  `demo` / `demo_name` に加えて **`demo_slug`**(どのメニュー行か)と
  **`demo_loop`**(ループ再生か ― 行が消されていてもループを続けるため)を書く。
  `show-run.json` を `show.json` より**先に**書くようにし、記録そのものが
  書けなかったときは古い記録を消すようにした(途中で電源が落ちても、前の
  ショーの記録が新しいショーと組にならない)。
  `restore()` はデモを**デモとして**戻す ― `running` だったものは焼き込み
  記録がそのショーを指していれば**そのまま走り直し**(PC のショーとまったく
  同じ復帰経路。**再焼き込みは無い**)、読み込んだだけなら `loaded` のまま。
  ログは `resumed the demo after a restart: cue …, no standby`
- **LCD が自分のデモを取り戻す**。`ui/main.py` は `restore()` のあとに App を
  作るので、App 側に `_adopt_restored_demo()` を足した:走って戻ってきた
  デモは DEMO 画面・ループ・名前・カーソル位置・KEY1 hold の再スタートごと
  引き継ぐ。slug を持たない 1 つ前のリリースの記録でも、保存済みデモの
  `show_id` が一致する行を探して戻す。読み込んだだけで戻ったデモはメニューの
  まま(ガーメントには何も出ていない)
- **KEY1 のガードを本当に必要な場合だけに**(`_pc_show_wins()`)。断るのは
  PC が本当にその機体を握っている 3 つの場合 ― ①ショーが running / holding、
  ②`restore()` がガーメントに戻したショーが待機中(hold されていた、または
  T0 を PC に確認してもらう途中。`state` は `loaded` に見えるが絵は出ている)、
  ③絵を書き込み中(`burn.state: burning` ― PC の Upload が飛んでいる。
  KEY1 はその焼き込みを取り消してしまう)。それ以外の loaded / stopped /
  ended はデモで上書きしてよい ― ただし**戻すにはもう一度 Upload が要る**
  (デモが走っているあいだ監視は何もしない)。注記は **5 秒**に伸ばし
  (オペレーターは 1.3 インチの画面ではなく壁を見ている)、文言も
  「PC show running - stop it on the PC」「PC is writing pictures - wait」に
- **PC 側は変更なし**。`conductor/fleet.py` は `/status` のフィールドしか
  見ていないので、機体が正しく名乗れば `_playing_demo` / `_burning_demo` は
  そのまま「触らない」を続ける ― 復帰したデモでそれを確かめるテストを追加
- ドキュメント:README の LCD 操作表のあと、CONDUCTOR_START §7 の手順 4 に
  「スタンドアロンデモは再起動・停電後に自動で再開する。止めるのは KEY2 か
  PC の STOP。」
- テスト:`tests/test_ui_demos.py` に 14 件(走行中の再起動 → デモのまま再開・
  再焼き込み無し、読み込みだけの再起動 → `loaded` のデモ、LCD が DEMO 画面ごと
  引き継いでループも続く、PC ショーが loaded なら KEY1 が通り running なら
  断られる、`demo` キーの無い古い記録 = PC ショー、別ショーを指す記録 = PC ショー、
  hold のまま再起動した PC ショーは KEY1 から守られる、焼き込み中の KEY1 は
  断られる、slug 無しの記録でも行が見つかる、行を消されたループが記録どおり
  ループする、`show.json` を書く前に落ちた PC のロードがデモの記録を残さない、
  記録を書けなかった PC のロードは古い記録を残さない、終わりまで再生しきった
  復帰ショーは KEY1 を邪魔しない、App ができる前に終わったループも引き継がれる)

**全角数字のファイル名で bundle の取り込みが落ちていた(2026-09-26)
―― 全角はそのまま受け入れる**

デザイナーの bundle に `AZ271SD1305_color_１_HW_grid.csv`(配色案名が
**全角数字** U+FF11)という柄ファイルが入っていた。シミュレーター側は
派生した柄名に Unicode をそのまま残すので通っていたが、Conductor の
`import_bundle` / `_SAFE_NAME` が「unusable file name」で弾き、その柄を
指すキューが全部「design … is not loaded」になって衣装が暗いままだった。

**配線ナビは今後も配色案名に全角を使う**とのことなので(運用側の判断)、
**半角に直すのではなく、そのまま受け入れる**ように両側を揃えた:

- **正規化は NFC だけ**(合成のみ。**NFKC は使わない**)。`AZ271SD1305_１_HW.csv`
  も `柄A` も `夏_2` も打ったとおりに保たれる。NFC が要るのは Mac 由来の
  **NFD**(「が」= か + ゛)を同じ名前として 1 ファイルに落とすため
- **ファイル名の規則は `conductor/look.py` の `normalize_name()` /
  `name_problem()` ただ 1 つ**で、Conductor(`workspace_name()`)と
  シミュレーター(`SIM.look` 経由で `conventionalName()`)が**同じ文字列比較**で
  使う。goldens の `names` ケースが両言語を突き合わせる
  - **保つ**: NFC 後に残るもの全部。日本語の約物(`・ 、 （ ） ＋ ゚`)も全角英数も
    そのまま。U+3000 は半角スペースに、前後の空白は落とす
  - **断る**(化けさせず、理由を添えて拒否): パス区切り `/ \ ／ ＼`、
    改行と制御文字(`U+0000-001F` `U+007F-009F` に **`U+2028` / `U+2029`** を含む
    ―― この 2 つは JS の正規表現では改行扱いで `.` が拾わず、Python では拾うので、
    同じ名前を両側が別に読んでしまう)、Windows が予約する `: * ? " < > |`、
    先頭/末尾のドット。
    **全角の `：＊？＂＜＞｜` は普通の文字として通す**(NTFS も配線ナビも問題にしない)。
    `／＼` だけ全角も断るのは、見た目がパスで、下流が幅を折りたたむと本物の
    区切りになりうるため
  - 以前の `safe_name()` は**置換**していたので `柄・A`・`柄　A`・`柄＋A` が
    1 つの `柄_A` に潰れ、3 つの柄が黙って上書きし合っていた(指摘 1)。
    いまは置換ではなく**拒否**
- **取り込み**: 合成(NFC)だけが許される変更。変わったものは結果の `renamed` に
  `{元の名前: 直した名前}` で載り(操作画面のトーストにも出る)、その名前を指す
  **キューの `design`・`transitions` のキー、そして map の改名なら `cues[].item`・
  `units`・`labels`・`boards` のキーも一緒に書き換える**ので参照が切れない(指摘 4)。
  合成すると**同じ名前になる 2 つ**は、後勝ちで黙って上書きせず、両方の綴りを
  挙げて片方を断る(指摘 3)
- **ファイル名から数字を読むところは全部 `[0-9]`**。Python の `\d` は全角数字を
  拾い JS の `\d` は拾わないので、`pattern１` が片側だけ P01 になっていた(指摘 2)
- **`_HW` は大文字だけ**。`my_notes_hw.csv` が「my」の柄「notes」になっていた。
  `<型番>_map_HW.csv` のように map と柄を兼ねる名前はどちらでもない(指摘 6)。
  Conductor と シミュレーターの「この 1 着に Add CSV」の**クライアント側の
  正規表現も大文字だけ**にした(`/i` のままだと `…_1_hw.csv` を衣装名に付け替えて
  送り、サーバーが断っていた)
- **前後の空白は「半角スペースだけ」を明示的に落とす**。`str.strip()` と
  `String.trim()` は端の扱いが違う(前者は `U+0085`・`U+001C-001F` を、後者は
  `U+FEFF` を食う)ので、どちらにも頼らない。それ以外の空白文字は全部制御文字なので
  **落とす前に拒否**する。`kind()` も `name_parts()` も**渡された名前そのもの**を
  検査する(整えた後の名前を見ると、拒否したい文字が消えてしまう)。ファイル名の
  stem も `Path.stem`(Windows では末尾のドットを落とす)ではなく
  シミュレーターと同じ `look.file_stem()` を使う
- **本番機に渡すラベルだけ**は折りたたむ(`showfile.unit_label()`)。機体の画面は
  `ui/render.py` が DejaVuSans で描いていて CJK の字形を持たないため、そのままだと
  豆腐が並ぶ。NFKC したうえで ASCII 0x20-0x7E の外を `?` に置換(連続は 1 個)
  ―― 柄「１」は `1`、柄「柄A」は `?A`。**ワークスペース・show.json・`cue.design` の
  参照・Conductor の画面は元の全角のまま**で、`ui/` には手を入れていない
  (機体の再配備は不要)

**既知のまま(指摘 7、直していない)**: 機体の画面でだけ**同じに見える柄**が
作れてしまう。`…_1_HW.csv` と `…_１_HW.csv` はどちらも `1`、漢字だけの柄名は
どれも `?` になるため、機体の LCD では見分けがつかない。ワークスペースでも
Conductor でも Timeline でも別物として正しく扱われるので実害は
...
きっかけ:デザイナーが LOOK 25 のシミュレーターに
`AZ271SD1301_1_HW.csv`(31 列・表裏とも 0〜18 行、全セル 0x04)を入れた。
この衣装の今の配線図は前 0〜33 行・後 0〜34 行・30 列(1482 スケール)で、
ファイルは**その衣装の古いレイアウト**から作られていた。当時の CHECK は
「部分的なデザインです」と「行シフトが 14 行で map と違う」しか出さず、
どちらも正しいが役に立たない ― ドレスの上半分が色無しで描かれ、
デザイナーはローダーが壊れたと思った。

直したこと:

- `conductor/look.py` の `check()` に `geometry_problem()` を足した。
  グリッドの**形**(どの行があるか、列がいくつか)だけを見て、
  合わないときは**先頭に 1 行**の問題を出す:
  ...
  発火条件は 3 つ、いずれも**セルの中身とは無関係**(未定 `-` や空セルが
  多いだけの「本当に部分的な柄」は決して引っかからない):片側で map の
  最上段より **2 行以上かつ 1 割超**足りない / map に無い行(または側)を
  持っている / map の最大列より**狭い**。**広い**のは正常
  (AZ271SD1307 の正規グリッドは 30 列の map に対して 31 列ある)
- `conductor/web/sim/model.js` に同じものを移植(goldens 再生成、
  ブラウザ自己テストで文字列まで一致を確認)。Conductor の Designs タブと
  シミュレーターの CHECK カードは同じ `check()` を出すので、
  両方に同じ 1 行が出る
- 配線ナビの「HW 用 CSV」ボタンが書き出す `<型番>_<配色案名>_HW.csv` を
  柄として**そのまま**受け付けるようにした(`look.kind()` /
  `Design.name_parts()`、model.js の同名、`/api/files`、Conductor と
  シミュレーターの「Add CSV」)。`AZ271SD1301_1_HW.csv` は
  `AZ271SD1301_color_1_grid.csv` と全く同じに扱われ、Designs には柄「1」
  として並ぶ。配色案名に `_` が入る場合(`summer_2`)は、すでに map がある
  衣装のうち**いちばん長く一致するもの**を型番とする。従来の名前も従来どおり
