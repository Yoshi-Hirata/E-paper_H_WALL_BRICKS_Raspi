...
1. Units タブでタイムラインに問題が無いことを確認(① Upload が押せる状態なら OK)
...
3. **Write demo to units** → 確認ダイアログで OK。機体ごとに OK / FAILED が出る
   (オフラインの機体は書き込まれない。あとで online になってから改めて書き込む)
...
6. デモの再生中はその機体に PC から Upload / START が届かない。Units タブの
   その機体の「Show」行に `demo` と出ていたら、まず **STOP** を押す(機体の
   デモも一緒に終わる)。それから改めて Upload・START すればよい
