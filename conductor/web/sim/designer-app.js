...
(function () {
  ...
  const clean = msg => rewriteCueLimitMessage(msg) ?? deJargon(labelize(msg));
  ...
  function displayCheck() {
    ...
    const result = { ok: failures.length === 0, total: dirty.length + cleanDirty.length + Object.keys(seqs).length, failed: failures.length, failures };
    try {
