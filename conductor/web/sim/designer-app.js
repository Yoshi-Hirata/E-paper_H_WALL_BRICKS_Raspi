...
(function () {
  ...
  // from 0.00, so that move gets its own controls rather than only the
  // click-an-empty-spot one: a "+" at the end of every track row and a link
  // in EDIT CUE, both landing here.
  //
  // The rotation is the item's own design list: the design AFTER the one the
  // cue being followed shows, wrapping round at the end - so pressing "+"
  // repeatedly walks the whole list and starts over, which is what a show
  // that cycles a garment's patterns actually wants. (The free click keeps
  // nextUnusedDesign(): it is a first placement, not a continuation.)
  function designAfter(item, afterName) {
    const list = (item && item.designs) || [];
    if (!list.length) return null;
    const i = list.findIndex(d => d.name === afterName);
  ...
  // here.
  //
  // CEIL, not round (review F1): complete + gap is the model's own legal
  // floor, not a target to land near, and rounding falls BELOW it for every
  // refresh whose fraction is under .5 - a perfectly ordinary Default
  // refresh time of 7.4 s gives complete + 1 = 8.4, which rounds to 8 and is
  // then flagged "only 8.0 s after the previous send; at least 8.4 s is
  // needed". The toFixed(3) first is for the same reason model.js rounds to
  // 3 decimals everywhere: complete is a sum of floats, and a 8.400000000001
  // must not ceil to 9.
  function appendTimeAfter(cue) { return Math.ceil(+(cue.complete + APPEND_GAP_S).toFixed(3)); }
  function appendDesignAfter(itemKey, cue) {
    const item = state.items.find(i => i.item === itemKey);
    if (!item) return;
    const design = cue ? designAfter(item, cue.design) : nextUnusedDesign(item);
    if (!design) {
      // Same refusal as the free click: a garment can have a map and no
      // design CSV yet, and a cue with design:null renders as a problem on
      // every later redraw instead of showing anything.
      toast("Load a design CSV for this item first.");
      return;
    }
    const at = cue ? appendTimeAfter(cue) : 0;
    if (at > MAX_SHOW_DURATION_S) {
      toast(`The show cannot run past ${clockShort(MAX_SHOW_DURATION_S)}, so there is no room after ${clockShort(cue.complete)}.`);
      return;
    }
    // A design placed at (or past) the end of the show would never be seen,
    // so the show grows to fit it - said out loud, because Show length is a
    // number the designer set on purpose and a silent change to it is the
    // kind of thing nobody notices until the run-through.
    if (at >= state.show.duration) {
      const grown = Math.min(MAX_SHOW_DURATION_S, Math.round(at + Math.max(state.show.refresh_s, APPEND_GAP_S)));
      globalThis.SIM.app.setShow({ duration: grown });
      toast(`Show length extended to ${clockShort(grown)} to fit the new design.`);
    }
    // Appending from EDIT CUE can insert in the MIDDLE of a track (the cue
    // it has open need not be the last one), and the cue that used to follow
    // can be pushed into "starts before the previous picture is complete" by
    // the new one landing in front of it. The editor then helpfully switches
    // to the freshly-added cue, which is clean - so the problem would appear
    // only as a red border further down the track and a number in the CUES
    // table (review F3). Compare this item's other cues either side of the
    // insertion and say so.
    const mine = () => state.show.cues.filter(c => c.item === itemKey);
    const hurtBefore = new Map(mine().map(c => [String(c.id), (c.problems || []).length]));
    const id = globalThis.SIM.app.addCue({ item: item.item, at, design: design.name, partial: false });
    ui.cue = id; render();
    const hurt = mine().filter(c => !sameId(c.id, id)
                              && (c.problems || []).length > (hurtBefore.get(String(c.id)) || 0))
                       .sort((a, b) => a.at - b.at)[0];
    if (hurt) {
      // The model's own sentence, not a paraphrase, and through clean() like
      // every other problem this page prints - some of them name a garment.
      const said = (hurt.problems || [])[hurt.problems.length - 1];
      toast(`Added at ${clockShort(at)} - but the design at ${clockShort(hurt.at)} on this item `
            + `now has a problem${said ? ": " + clean(said) : ""}.`);
    }
    return id;
  }
  ...
  function renderCueEditor(cue) {
    ...
    if (!cue) { card.innerHTML = `<div class="meta">Select a cue - or click a track to place a design there, or press <b>+</b> at the end of a track row to add the next one.</div>`; return; }
    ...
    card.innerHTML = `<div class="cue-card">
      ...
      <div class="cue-append"><button type="button" id="cue-append" class="linkish"
        title="Place this item's next design as soon as this one has finished, at ${esc(clockShort(appendTimeAfter(cue)))}">＋ Add next design after this cue</button></div>
    ...
    $("#cue-append").onclick = () => appendDesignAfter(cue.item, cue);
  ...
  function renderTracks() {
    ...
    // A width is clamped against what is LEFT of the track, not against the
    // whole of it (review F4): a cue whose complete or end passes the end of
    // the show clamped left and width independently, so left + width came to
    // more than 100% and the band ran out under the "+" column.
    const pctW = (t, from) => (100 * Math.max(0, Math.min(D - Math.max(0, Math.min(D, from)), t)) / D).toFixed(3) + "%";
    const rows = items.map(item => {
      const cues = cuesOf(item);
      const bands = cues.map(cue => {
        const bandW = pctW(Math.max(0, cue.complete - cue.at), cue.at);
        const holdX = pct(cue.complete), holdW = pctW(Math.max(0, cue.end - cue.complete), cue.complete);
        // A hold with no room left in the show is pinned to the right edge
        // rather than placed at left:100% (review F4's other half): it still
        // takes up its own 2 px border plus 5 px of padding whatever its
        // width, so a zero-width one at 100% hangs 7 px out under the "+".
        const nub = parseFloat(holdW) <= 0;
        ...
        return `<div class="cue-band ${sameId(cue.id, ui.cue) ? "sel" : ""}" data-cue="${esc(cue.id)}" style="left:${pct(cue.at)};width:${bandW}"></div>
          <div class="cue-hold ${bad ? "bad" : ""} ${sameId(cue.id, ui.cue) ? "sel" : ""}" data-cue="${esc(cue.id)}" data-drag="${cue.at <= 0 ? "0" : "1"}"
            style="${nub ? "right:0" : `left:${holdX}`};width:${holdW}" title="${esc(designLabel(item.designs.find(d => d.name === cue.design) || { name: cue.design }))}">${cue.at <= 0 ? "PRESET · " : ""}${esc(designLabel(item.designs.find(d => d.name === cue.design) || { name: cue.design }))}</div>`;
      }).join("");
      // An item with nothing on it says so INSIDE its own track, where the
      // click that would fix it has to happen - the one hint line above the
      // tracks was easy to read past (user, 2026-09-25). pointer-events are
      // off in the CSS, so the words never swallow that click.
      const emptyHint = cues.length ? ""
        : `<div class="tl-empty">click here to place the first design at ${esc(clockShort(0))}</div>`;
      const last = cues[cues.length - 1];
      const addTitle = last
        ? `Add the next design after the last cue (at ${clockShort(appendTimeAfter(last))})`
        : `Add the first design (at ${clockShort(0)})`;
      return `<div class="tl-row"><div class="tl-name" title="${esc(itemFull(item))}">${esc(itemName(item))}<small>${esc(itemSub(item))}</small></div>
        <div class="tl-track" data-track="${esc(item.item)}">${bands}${emptyHint}</div>
        <button type="button" class="tl-add" data-add="${esc(item.item)}" title="${esc(addTitle)}"
          aria-label="${esc(itemName(item))}: ${esc(addTitle)}">+</button></div>`;
    }).join("");
    ...
    // are 150 px (about 1.5 min on a 10 min show) left of every cue. Every
    // row now also ends with a --tl-add-w column (the "+" button), so the
    // ruler row carries a matching empty cell and the playhead wrapper stops
    // short of it by the same amount, or both would run 32 px past the tracks.
  ...
  function cueTable() {
    ...
    // A brand-new project's biggest question is "where do I start?", and the
    // CUES card is the widest empty space on the screen to answer it in - so
    // the same sentence the empty tracks carry is repeated here, full size.
    if (!rows.length) return `<div class="tl-firsthint">Nothing placed yet - click the start of a track above to place the first design at ${esc(clockShort(0))},
      or press <b>+</b> at the end of that track's row to place it for you.</div>`;
  ...
  function renderTimelineTab() {
    ...
    root.innerHTML = `<div class="toolbar">
      ...
      ${items.length ? `<div id="tl-editing">
        <div class="tl-left">
          <div class="card"><div class="tl-hint">Click anywhere on a track to place a design at that moment, or press <b>+</b> at the end of a row to add that item's next design straight after its last one.</div>${renderTracks()}</div>
          ${warningsBlock()}
  ...
  // ---- the hover ghost (user, 2026-09-25) ---------------------------
  // "Click an empty spot on a track" was true and invisible: nothing on the
  // screen said a click would do anything until it already had. Moving the
  // pointer over empty track space now draws the cue that click would
  // create - at the same snapped moment, showing the same design name - so
  // the gesture is discovered by hovering rather than by reading.
  //
  // One ghost exists at a time, it is pointer-events:none (so it can never
  // become e.target and confuse the click/hover checks under it), and it
  // lives inside the track's own innerHTML, which every render replaces
  // wholesale - so no cleanup is owed on redraw, only on the three "not a
  // placement any more" cases below.
  function clearGhost() {
    document.querySelectorAll(".cue-ghost").forEach(g => g.remove());
    document.querySelectorAll(".tl-track.has-ghost").forEach(t => t.classList.remove("has-ghost"));
  }
  function trackTimeAt(track, clientX) {
    const box = track.getBoundingClientRect();
    const D = state.show.duration;
    if (!box.width || !D) return 0;
    return Math.round(Math.max(0, Math.min(D, (clientX - box.left) / box.width * D)));
  }
  function moveGhost(track, e) {
    // Never while a cue is being dragged or the playhead is being scrubbed
    // (both are pointer gestures that pass right over empty track space),
    // and never on top of an existing cue, where a click selects rather
    // than places.
    if (cueDrag || (transport && transport.seeking)) { clearGhost(); return; }
    if (e.target.closest(".cue-hold") || e.target.closest(".cue-band")) { clearGhost(); return; }
    const item = state.items.find(i => i.item === track.dataset.track);
    const design = item && nextUnusedDesign(item);      // exactly what the click would place
    if (!design) { clearGhost(); return; }
    const D = state.show.duration;
    const t = trackTimeAt(track, e.clientX);
    let ghost = track.querySelector(".cue-ghost");
    if (!ghost) {
      clearGhost();
      ghost = document.createElement("div");
      ghost.className = "cue-ghost";
      ghost.appendChild(document.createElement("span")).className = "cue-ghost-label";
      track.appendChild(ghost);
      track.classList.add("has-ghost");
    }
    ghost.style.left = (100 * t / D).toFixed(3) + "%";
    ghost.style.width = (100 * Math.min(Math.max(0, D - t), state.show.refresh_s) / D).toFixed(3) + "%";
    ghost.dataset.at = String(t);
    const label = ghost.firstChild;
    label.textContent = `+ ${designLabel(design)} at ${clockShort(t)}`;
    // The label hangs off the right of the band, and flips to its left when
    // it would otherwise run past the end of the track. MEASURED, not
    // guessed from t (review F2): a design name is anywhere from 6 to 40
    // characters, so a fixed "past 80% of the show" threshold both flips too
    // early for a short name and lets a long one run tens of pixels past the
    // card before it. Text first, then measure, then decide.
    label.classList.remove("flip");
    if (label.getBoundingClientRect().right > track.getBoundingClientRect().right) label.classList.add("flip");
  }
  function wireTrackEvents() {
    document.querySelectorAll(".tl-track").forEach(track => {
      track.addEventListener("pointermove", e => moveGhost(track, e));
      track.addEventListener("pointerleave", clearGhost);
      track.addEventListener("pointerdown", clearGhost);
    ...
    // The per-row "+" - a real <button>, so it is in the tab order and
    // answers Enter and Space without any handler of its own (and the
    // Space-is-play shortcut already steps aside for a focused button).
    document.querySelectorAll("button.tl-add[data-add]").forEach(btn => btn.addEventListener("click", e => {
      e.stopPropagation();
      clearGhost();
      const item = state.items.find(i => i.item === btn.dataset.add);
      appendDesignAfter(btn.dataset.add, item && lastCueOf(item));
    }));
  ...
  function renderHelp() {
    $("#content").innerHTML = `<div class="help">
      ...
      <p>Double-click <code>az27ss-simulator.html</code> (or <code>designer.html</code> during development) - no install, no server. Drop the garments' map and design CSV files anywhere on this page to begin (a folder works too), or use <b>Add CSV</b> in the header. The <b>Designs</b> tab has an <b>Add CSV</b> of its own under DESIGNS OF THIS ITEM, which adds the files you pick to that one garment: whatever they are named, they are saved under that garment's name and belong to it alone. A <code>*_map.csv</code> that belongs to another garment already in the project is refused there - a garment's wiring is not interchangeable the way its patterns are - so use the header's <b>Add CSV</b> for that one. Open the <b>Timeline</b> tab to place the designs (see below).</p>
      <h2>Timeline: adding designs one after another</h2>
      <p>A show is normally built from ${esc(clockShort(0))} outwards, one design after the next, and there are two ways to add one. <b>Click anywhere on an item's track</b> to place a design at that moment: as the pointer moves over empty track space, a faint band shows exactly where the design would land and which one it would be ("+ name at m.ss"), and the click places it. Or press the <b>+</b> at the right-hand end of a track row: it adds that item's <b>next design</b> - the one after the last cue's in that item's list, starting over at the top when it runs out - as soon as the previous one has finished being drawn (its complete time plus one second). Press it again and again to lay a whole rotation out along the track. If the new design would fall past the end of the show, <b>Show length</b> is extended to fit and you are told so. <b>EDIT CUE</b> carries the same thing as <b>＋ Add next design after this cue</b>, relative to the cue you have open, and the newly added cue is always the one that opens there.</p>
      <p>Two things worth knowing about the <b>+</b>: on a track with nothing on it yet the first press places the design at ${esc(clockShort(0))}, which makes it that item's <b>preset</b> - what it shows before the show starts - so press it again for the first design of the show proper, one second later; and an item that has only one design keeps being given that same design, since the rotation has nowhere else to go.</p>
      <p>Click an existing cue to edit it, or drag it to move it (hold Shift for 5 s steps instead of 1 s).</p>
  ...
  async function addFilesFromBlobs(files) {
    ...
    const list = [], refused = [];
    const skipped = [], readAs = [], takenNames = new Set();
